...
if not st.session_state.get('is_logged_in', False):
    ...
    # 로그인되지 않은 경우 기능 비활성화
    st.sidebar.text_input("새 카테고리 이름", disabled=True)
    st.sidebar.button("카테고리 추가", disabled=True)
    st.selectbox("카테고리를 선택하세요", [], disabled=True)
    st.text_input("제목", disabled=True)
    st.text_area("오늘 하루 있었던 일", height=200, disabled=True)
    st.file_uploader("Choose an image", type=["jpg", "jpeg", "png"], disabled=True)
    st.button("확인", disabled=True)
else:
    # 표정 이미지와 레이블 정의
    expressions = [
        ('간지.png', '간지'),
